...
- **Python 3.13** - Core affordability calculations
- **NumPy/Pandas** - Data processing (optional)
- **JSON** - Company and configuration data
...
- **Python 3.13+** with venv (for calculation engine)
- **Node.js 18+** and npm (for UI)
- **Neon Postgres database** (for UI persistence)
...
│   └── schema.sql
├── calc_affordability.py  # Python calculation engine
├── rate_sensitivity.py    # Rate scenario analysis
├── validate_data.py       # Data validation scripts
├── test_outputs/         # Python test outputs
...
- **Aggregation & overview helpers (Techridge-wide, demand + supply)**
  - `compute_techridge_demand(companies, years, scenarios, affordability_lookup)`
    - Aggregates demand across all companies loaded from `data/` to get **Techridge-wide households by product** for each **year, scenario, and rate**.
  - `load_supply()` and `compute_supply_by_year()`
    - Read `data/supply.json` and compute **available units per product per year** based on `first_delivery_year`.
  - `build_techridge_overview_json(years, scenarios)`
    - Returns a compact JSON structure containing:
      - `demand_by_product` (Techridge-wide, by year/scenario/rate); and
      - `supply_by_product` (units per product and year),
    - designed to be consumed directly by the Next.js Overview and Graphs pages without exposing band-level internals.
...
The core affordability engine is in `lib/model.ts`. It's a TypeScript port of the Python implementation.
...
## 11. Supply Modeling & Frontend Structure

To avoid clutter and over-design in the interface, we separate **core overview metrics** from **deeper visualizations** and treat supply inputs as a simple, explicit config.

### 11.1 Supply Configuration (Manual, Simple)

- **`data/supply.json`** holds planned unit counts and delivery timing:
  - For each product:
    - `name`: `"Apartments" | "Condos" | "Blackridge" | "Townhouse"`.
    - Units by phase with delivery years
    - Planning notes and metadata
- The modeling side (Python) already:
  - Computes **demand households** by product and year (from the affordability engine) via `compute_techridge_demand`.
  - Compares against **available supply units** per year (from `compute_supply_by_year`) so the frontend can show **gaps** (demand minus supply) without hard-coding supply logic in the UI.

### 11.2 Clean UI Structure

To keep the app clean and avoid crowding the home page, the frontend is organized around focused views:

- **Overview (`/` or `/overview`)**
  - Purpose: quick, non-cluttered snapshot of Techridge-wide demand.
  - Content:
    - 4 product cards (Apartments, Condos, Blackridge, Townhouses) showing:
      - Demand households (for selected rate + year).
      - Supply units and a simple gap indicator (e.g. +40 over-subscribed).
    - Compact company list (busybusy, Vasion, Zonos, etc.) with minimal stats, linking to detail pages.
    - Controls: interest-rate slider, income scenario toggle (Base vs Base+Bonus), and year selector (2025/26/27+) – nothing more.

- **Companies (`/companies/[slug]`)**
  - One page per company (e.g. `/companies/vasion`, `/companies/busybusy`) that:
    - Starts with a few key numbers (households by product, band mix).
    - Includes at most **one or two charts** (e.g. demand by product over time) plus a simple table.
    - Avoids duplicating global controls; inherits the current rate/income/year from the top-level UI.

- **Graphs / Analysis (`/graphs` or `/analysis`)**
  - A **dedicated visualization page** instead of cramming charts into the home view.
  - Focused on 2–3 high-value charts such as:
    - Demand vs **supply over time** by product.
    - Rate-sensitivity curves for a chosen product (e.g. condos) or company.
  - This page can be optionally hidden from casual users; the Overview stays simple.

- **Assumptions & Bands (`/assumptions`, optional `/income-bands`)**
  - Lightweight forms / tables for:
    - Editing rates, DTI, tax/HOA %, and **supply counts + delivery years**.
    - Viewing income band definitions and maybe a single stacked bar chart of household distribution across B1–B7.
  - These routes are intentionally **text- and table-heavy**, with minimal charts, to keep the main Overview and company pages visually clean.

This separation lets the UI **scale** (more companies, more scenarios, richer charts) without overloading the homepage: the Overview stays a calm summary, while deeper pages handle detail and visual exploration.

---

## 12. Rate Data Implementation Plan (Frontend & Backend)
...
### 12.2 Next.js UI (Hosted on Vercel)
...
1. **Phase 1 – Fetch-on-demand (no DB)**
   - Implement `GET /api/fha-rate` in Next.js:
     - Attempts to fetch FHA 30‑yr from a trusted source (e.g. MND via scraping/API or reused `useMortgageRates` logic adapted to Node).
